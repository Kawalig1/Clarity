...
public class REGPH1033Verifyingmysavingspageformemberwithonebenefitsanduserispreserved {
  ...
  @Test
  public void testREGPH1033Verifyingmysavingspageformemberwithonebenefitsanduserispreserved() throws Exception {
    ...
    // Warning: verifyTextPresent may require manual changes
    try {
      assertTrue(driver.findElement(By.cssSelector("BODY")).getText().matches("^[\\s\\S]*£27,500\\.00[\\s\\S]*$"));
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    // Warning: verifyTextPresent may require manual changes
    try {
      assertTrue(driver.findElement(By.cssSelector("BODY")).getText().matches("^[\\s\\S]*The amount that you and your employer save is not taxed\\. The cost to you is £8,800\\.00 if you pay basic-rate tax or £6,600\\.00 if you pay higher-rate tax\\.[\\s\\S]*$"));
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    // Warning: verifyTextPresent may require manual changes
