...
public class ATS002EmployerabletoimpersonateaPensionerwithloginandstatusPENSUS {
  ...
  @Test
  public void test0207ATS002EmployerabletoimpersonateaPensionerwithloginandstatusPENSUS() throws Exception {
    ...
    try {
      assertTrue(driver.findElement(By.cssSelector("BODY")).getText().matches("^[\\s\\S]*PensionerDevFive[\\s\\S]*$"));
    } catch (Error e) {
    ...
    // Warning: verifyTextPresent may require manual changes
    try {
      assertTrue(driver.findElement(By.cssSelector("BODY")).getText().matches("^[\\s\\S]*02\\.08\\.05[\\s\\S]*$"));
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    // Warning: assertTextPresent may require manual changes
